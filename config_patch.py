...
def patch_configuration_yaml(hass: HomeAssistant) -> bool:
    ...
    try:
        ...
        if bool(http.get("use_x_forwarded_for")) is not True:
        ...
        def _contains(seq, value: str) -> bool:
            """Return True if seq contains value (string compare, ruamel scalars safe)."""
            return any(str(x) == value for x in seq)

        for w in wanted:
            if not _contains(tp, w):
                tp.append(w)
                changed = True
